...
describe('Testing Usuarios CRUD', ()=>{
  ...
  beforeAll(async ()=>{
    jest.resetModules();
    process.env = {
      ...env, //Copia uno por uno todo de los atributos al objeto process.env y agg los otros 2 de abajo
  ...
  afterAll(async ()=>{
    ...
    process.env = env; // Se deja el objeto como estaba
    return true;
  });
  ...
  test('Usuarios updateOne Record', async ()=>{
    ...
    const updatedRecord = await Usr.updateUsuarios({
      codigo:2,
